...
export const CATEGORY_CONFIG: Record<string, { color: string; subcategories: string[] }> = {
  ...
  'Beer & Cider': {
    color: '#c2a46a',
    subcategories: ['Lager', 'Stout', 'Malt beer', 'Light beer', 'Craft beer', 'Cider', 'Alcopops'],
  },
  'Soft Drinks': {
    color: '#d7956c',
    subcategories: [
  ...
  Mixers: {
    color: '#9c97ba',
    subcategories: ['Tonic', 'Soda water', 'Ginger ale', 'Cola mixers', 'Other mixers'],
  },
  'Snacks & Accessories': {
    color: '#ae9c83',
    subcategories: ['Snacks', 'Ice', 'Cups', 'Bottle openers', 'Other accessories'],
  },
};
...
const glovo =
  'https://glovoapp.com/ke/en/nanyuki/chandarana-foodplus-nanyuki-nyk/?content=alcohol-sc.12803164/the-spirits-c.12803161';
group(
  'Spirits',
  glovo,
  `Smirnoff Red Label|Smirnoff|Vodka|750ml
...
group(
  'Spirits',
  'https://www.oaks.delivery/product/johnnie-walker-black-label-1l/',
  `Johnnie Walker Black Label|Johnnie Walker|Whisky|250ml
...
group(
  'Spirits',
  'https://www.oaks.delivery/product/johnnie-walker-red-label-375ml/',
  `Johnnie Walker Red Label|Johnnie Walker|Whisky|250ml
...
group(
  'Spirits',
  'https://nairobidrinks.co.ke/whisky/',
  `Glenfiddich 15 Year Old|Glenfiddich|Whisky|1L
...
group(
  'Spirits',
  'https://www.oaks.delivery/product/konyagi-750ml/',
  `Konyagi|Konyagi|Imported spirits|250ml
...
group(
  'Spirits',
  'https://www.oaks.delivery/product-category/spirits/',
  `Kibao Vodka|Kibao|Vodka|750ml
Aviation American Gin|Aviation|Gin|750ml`,
);
group(
  'Beer & Cider',
  'https://www.oaks.delivery/product-category/spirits/',
  `Bacardi Breezer Strawberry|Bacardi|Alcopops|275ml`,
);
group(
  'Spirits',
  'https://www.carrefour.ke/mafken/en/c/FKEN21030500',
  `Amarula Cream Liqueur|Amarula|Cream liqueurs|750ml
Jägermeister Herbal Liqueur|Jägermeister|Liqueurs|1L
Baileys Original Irish Cream|Baileys|Cream liqueurs|375ml`,
);
group(
  'Wines',
  'https://nairobidrinks.co.ke/wine/',
  `4th Street Sweet Rosé|4th Street|Rosé|750ml
...
group(
  'Wines',
  'https://www.oaks.delivery/product/four-cousins-sweet-red-75cl/',
  `Four Cousins Natural Sweet Red|Four Cousins|Red wine|750ml
Four Cousins Natural Sweet Red|Four Cousins|Red wine|1.5L`,
);
group(
  'Wines',
  'https://www.oaks.delivery/c/wine/',
  `Mucho Más Black Edition|Mucho Más|Red wine|750ml
...
group(
  'Beer & Cider',
  beer,
  `Tusker Lager|Tusker|Lager|500ml|can
...
group(
  'Soft Drinks',
  soft,
  `Coca-Cola Original|Coca-Cola|Cola|350ml
...
group(
  'Energy Drinks',
  soft,
  `Red Bull Energy Drink|Red Bull|Energy drinks|250ml|can
...
group(
  'Mixers',
  soft,
  `Schweppes Tonic Water|Schweppes|Tonic|330ml|can
Schweppes Tonic Water|Schweppes|Tonic|500ml`,
);
group(
  'Mixers',
  'https://www.carrefour.ke/mafken/en/tonic-soda-water/schweppes-tonic-water-ginger330ml/p/178181',
  `Schweppes Ginger Ale|Schweppes|Ginger ale|330ml|can`,
);
group(
  'Water',
  'https://alladin.co.ke/alcoholic-beverages-cigarettes/drinks/water',
  `Dasani Drinking Water|Dasani|Bottled water|500ml
Keringet Sparkling Water|Keringet|Sparkling water|500ml`,
);
group(
  'Water',
  'https://www.tawalasupermarket.co.ke/water/154-dasani-mineral-water-15l-1000010000254.html',
  `Dasani Drinking Water|Dasani|Bottled water|1.5L
...
group(
  'Snacks & Accessories',
  'https://www.carrefour.ke/mafken/en/chips/trop-heat-crisps-salted-50g/p/32275',
  `Tropical Heat Salted Potato Crisps|Tropical Heat|Snacks|50g|bag`,
);
group(
  'Snacks & Accessories',
  'https://www.carrefour.ke/mafken/en/c/FKEN1730200',
  `Tropical Heat Chilli Lemon Potato Crisps|Tropical Heat|Snacks|50g|bag`,
);
// The opener's model is verified; no package count or dimensions are invented. Confirm the physical unit in Products before sale.
group(
  'Snacks & Accessories',
  'https://www.carrefour.ke/mafken/en/c/NFKEN8090909',
  `Homezaza Mini Bottle Opener DH1839|Homezaza|Bottle openers||unit`,
);

export function seedCatalogue(db: DB, a: Actor, includeNew = false) {
  const existing = one(db, 'SELECT id FROM products WHERE business_id=? LIMIT 1', a.business_id);
  if (existing && !includeNew) return;
  db.transaction(() => {
    let added = 0;
    for (const [name, config] of Object.entries(CATEGORY_CONFIG))
      if (!one(db, 'SELECT id FROM categories WHERE business_id=? AND name=?', a.business_id, name))
        insert(db, 'categories', { id: id('cat_'), business_id: a.business_id, name, color: config.color });
    for (const [index, p] of CATALOGUE.entries()) {
      ...
      if (!brand) {
        const brandId = id('brand_');
        insert(db, 'brands', { id: brandId, business_id: a.business_id, name: p.brand });
        brand = { id: brandId };
      }
      const category = one(
        db,
      ...
      if (
        one(
          db,
          'SELECT id FROM products WHERE id=? OR (business_id=? AND name=? AND size=? AND unit=?)',
          productId,
          a.business_id,
          p.name,
          p.size,
      ...
      insert(db, 'products', {
        ...
        sku: one(
          db,
          'SELECT id FROM products WHERE business_id=? AND sku=?',
          a.business_id,
          `KIL-${String(index + 1).padStart(5, '0')}`,
        )
          ? `KIL-${id().slice(0, 8).toUpperCase()}`
          : `KIL-${String(index + 1).padStart(5, '0')}`,
        size: p.size,
        unit: p.unit,
        source_url: p.source,
        notes:
          'Starter listing. Confirm the physical package, barcode and tax treatment before selling. Prices are intentionally unset.',
        created_at: now(),
        updated_at: now(),
      });
    ...
    if (added)
      audit(
        db,
        a,
        existing ? 'catalogue.extended' : 'catalogue.seeded',
        'products',
        a.business_id,
        null,
