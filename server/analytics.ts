...
export const shiftDate = (date: string, days: number) =>
  new Date(new Date(`${date}T12:00:00Z`).getTime() + days * 86400_000).toISOString().slice(0, 10);
export type Range = { from: string; to: string };
export function readRange(query: Record<string, unknown>): Range {
  const b = z
    .object({ from: dateInput.default(shiftDate(kenyaDate(), -6)), to: dateInput.default(kenyaDate()) })
    .parse(query);
  requireThat(b.from <= b.to, 'Start date must not be after end date.');
  requireThat(
    new Date(b.to).getTime() - new Date(b.from).getTime() <= 366 * 86400_000,
    'Select a date range of at most 366 days.',
  );
  return b;
}
export function bounds(r: Range) {
  return [
    new Date(`${r.from}T00:00:00+03:00`).toISOString(),
    new Date(`${shiftDate(r.to, 1)}T00:00:00+03:00`).toISOString(),
  ];
}
export function limited(rows: Row[]) {
  requireThat(rows.length <= 50000, 'Too many records. Narrow your report date range.', 413);
  return rows;
}
export function saleEvents(db: DB, a: Actor, r: Range): Row[] {
  ...
  const positive = all(
    db,
    `SELECT si.id event_id,s.id sale_id,s.ref transaction_ref,s.ref original_ref,s.created_at,'sale' event_type,
    ...
    WHERE si.business_id=? AND si.branch_id=? AND s.created_at>=? AND s.created_at<? LIMIT 50001`,
    a.business_id,
    a.branch_id,
    start,
    end,
  ...
  const negative = all(
    db,
    `SELECT ri.id event_id,s.id sale_id,r.ref transaction_ref,s.ref original_ref,r.created_at,'return' event_type,
    si.product_id,si.product_name,si.sku,si.size,si.brand_name brand,si.category_name category,si.supplier_name supplier,
    ...
    WHERE r.business_id=? AND r.branch_id=? AND r.created_at>=? AND r.created_at<? LIMIT 50001`,
    a.business_id,
    a.branch_id,
    start,
    end,
  );
  return limited([...positive, ...negative].sort((a, b) => b.created_at.localeCompare(a.created_at))).map(
    (row) => ({
      ...row,
      date: kenyaDate(new Date(row.created_at)),
      revenue_cents: row.total_cents - row.tax_cents,
      profit_cents: row.total_cents - row.tax_cents - row.cogs_cents,
    }),
  );
}
...
export function stats(db: DB, a: Actor, r: Range) {
  const events = saleEvents(db, a, r),
    expenses = expenseEvents(db, a, r),
    [start, end] = bounds(r);
  const transactions = one(
    db,
    'SELECT COUNT(*) count,COALESCE(SUM(total_cents),0) gross FROM sales WHERE business_id=? AND branch_id=? AND created_at>=? AND created_at<?',
    a.business_id,
    a.branch_id,
    start,
    end,
  )!;
  const otherCosts = one(
    db,
    `SELECT COALESCE(SUM(l.debit_cents-l.credit_cents),0) n FROM journal_lines l JOIN journal_entries e ON e.id=l.entry_id WHERE l.business_id=? AND l.branch_id=? AND e.created_at>=? AND e.created_at<? AND l.account IN('Inventory adjustments','Cash over / short')`,
    a.business_id,
    a.branch_id,
    start,
    end,
  ...
  return {
    ...
    average_cents: transactions.count ? Math.round(transactions.gross / transactions.count) : 0,
    discounts_cents: sum(events, 'discount_cents'),
  };
}
...
export function dashboard(db: DB, a: Actor, r: Range) {
  const events = saleEvents(db, a, r),
    products = productList(db, a),
    today = kenyaDate();
  const current = stats(db, a, r),
    [start, end] = bounds(r),
    yesterday = shiftDate(today, -1);
  const groups: Row[] = aggregate(events, 'product_id').map((g) => {
    const product = products.find((p) => p.id === g.name);
    return {
      ...g,
      ...(product
        ? {
  ...
  const previousEvents = saleEvents(db, a, { from: shiftDate(r.from, -days), to: shiftDate(r.from, -1) }),
    previousGroups = aggregate(previousEvents, 'product_id');
  const byDay = Array.from({ length: days }, (_, n) => {
    const day = shiftDate(r.from, n),
      data = events.filter((e) => e.date === day);
    return {
      date: day,
      sales_cents: sum(data, 'total_cents'),
      profit_cents: sum(data, 'profit_cents'),
      transactions: new Set(data.filter((e) => e.event_type === 'sale').map((e) => e.sale_id)).size,
    };
  });
  const payment = all(
    db,
    'SELECT method,SUM(amount_cents) amount_cents,COUNT(*) entries FROM payments WHERE business_id=? AND branch_id=? AND created_at>=? AND created_at<? GROUP BY method',
    a.business_id,
    a.branch_id,
    start,
    end,
  ...
  const inv = all(
    db,
    "SELECT date(created_at,'+3 hours') date,SUM(CASE WHEN quantity>0 THEN quantity ELSE 0 END) incoming,SUM(CASE WHEN quantity<0 THEN -quantity ELSE 0 END) outgoing FROM inventory_movements WHERE business_id=? AND branch_id=? AND created_at>=? AND created_at<? GROUP BY date(created_at,'+3 hours')",
    a.business_id,
    a.branch_id,
    start,
    end,
  ...
  const weekStart = shiftDate(today, -((weekday + 6) % 7));
  return {
    range: r,
    summary: current,
    today: stats(db, a, { from: today, to: today }),
    yesterday: stats(db, a, { from: yesterday, to: yesterday }),
    week: stats(db, a, { from: weekStart, to: today }),
    month: stats(db, a, { from: today.slice(0, 7) + '-01', to: today }),
    inventory: {
      value_cents: sum(products, 'stock_value_cents'),
      total_units: sum(products, 'stock'),
      low_stock: low.length,
      out_of_stock: products.filter((p) => p.active && p.stock === 0).length,
      low_products: low.slice(0, 5),
    },
    pending: one(
      db,
      "SELECT COUNT(*) n FROM approval_requests WHERE business_id=? AND branch_id=? AND status='pending'",
      a.business_id,
      a.branch_id,
    )!.n,
    cash_variance_cents: cashVariance,
    series: byDay,
    categories: aggregate(events, 'category'),
    brands: aggregate(events, 'brand'),
    payments: payment,
    top_products: groups.slice(0, 7),
    best_selling: [...groups].sort((a, b) => b.quantity - a.quantity).slice(0, 10),
    highest_margin: [...groups]
      .filter((p) => p.revenue_cents > 0)
      .sort((a, b) => b.margin - a.margin)
      .slice(0, 10),
    slow_moving: products
      .filter((p) => p.stock > 0 && !groups.some((g) => g.product_id === p.id && g.quantity > 0))
      .slice(0, 10),
    declining: previousGroups
      .filter((g) => g.quantity > 0)
      .map((g) => ({
        product_id: g.name,
        name: products.find((p) => p.id === g.name)?.name ?? g.name,
        previous_quantity: g.quantity,
        current_quantity: groups.find((p) => p.product_id === g.name)?.quantity ?? 0,
      }))
      .filter((g) => g.current_quantity < g.previous_quantity),
    staff: staffPerformance(db, a, r),
    inventory_series: byDay.map((d) => ({
      date: d.date,
      incoming: inv.find((i) => i.date === d.date)?.incoming ?? 0,
      outgoing: inv.find((i) => i.date === d.date)?.outgoing ?? 0,
    })),
    recent_sales: all(
      db,
      'SELECT s.*,u.name staff_name FROM sales s JOIN users u ON u.id=s.user_id WHERE s.business_id=? AND s.branch_id=? ORDER BY s.created_at DESC LIMIT 5',
      a.business_id,
      a.branch_id,
    ),
    activity: all(
      db,
      "SELECT l.id,l.action,l.created_at,l.reason,u.name user_name FROM audit_logs l JOIN users u ON u.id=l.user_id WHERE l.business_id=? AND l.branch_id=? AND l.action NOT LIKE 'auth.%' ORDER BY l.seq DESC LIMIT 5",
      a.business_id,
      a.branch_id,
    ),
    setup: {
      products: products.length,
      priced: products.filter((p) => p.cost_cents !== null && p.selling_cents !== null).length,
      tax_configured: products.filter((p) => p.tax_mode !== 'unset').length,
      stocked: products.filter((p) => p.stock > 0).length,
      ready: products.filter(
        (p) => p.stock > 0 && p.selling_cents !== null && p.cost_cents !== null && p.tax_mode !== 'unset',
      ).length,
      sales: one(
        db,
        'SELECT COUNT(*) n FROM sales WHERE business_id=? AND branch_id=?',
        a.business_id,
        a.branch_id,
      )!.n,
      admins: one(
        db,
        "SELECT COUNT(*) n FROM users WHERE business_id=? AND branch_id=? AND active=1 AND role_id IN('admin','super_admin')",
        a.business_id,
        a.branch_id,
      )!.n,
    },
    catalogue_categories: Object.entries(
...
export function installAnalytics(app: Express, db: DB) {
  app.get('/api/dashboard', protect('dashboard.read'), (req, res) =>
    res.json(dashboard(db, req.actor, readRange(req.query))),
  );
  app.get('/api/analytics/staff', protect('staff.read'), (req, res) =>
    res.json({
      staff: staffPerformance(db, req.actor, readRange(req.query)),
      notice: 'Operational data for human review. These figures are not evidence of misconduct.',
    }),
  );
  app.get('/api/intelligence/features', protect('reports.read'), (req, res) => {
    const r = readRange(req.query);
    res.json({
      schema_version: 1,
      read_only: true,
      range: r,
      metrics: stats(db, req.actor, r),
      product_features: aggregate(saleEvents(db, req.actor, r), 'product_id'),
      policy: 'Recommendations only. This interface cannot modify financial records.',
    });
  });
}
