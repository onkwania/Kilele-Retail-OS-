...
export function newRequest(
  ...
  if (input.evidence_id) {
    const doc = scoped(db, 'documents', input.evidence_id, a);
    requireThat(doc.user_id === a.id, 'Supporting evidence must be uploaded by you.', 403);
  }
  const row = {
    id: id('req_'),
    ref: ref('RQ'),
    ...
    original_json: JSON.stringify(input.original ?? {}),
