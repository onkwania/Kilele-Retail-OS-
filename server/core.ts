...
export function verifyAudit(db: DB) {
  ...
  for (const row of all(db, 'SELECT * FROM audit_logs ORDER BY seq')) {
    ...
    if (entry.previous_hash !== previous || sha(JSON.stringify(entry)) !== hash)
...
export function integrity(db: DB) {
  ...
  if (
    all(db, 'SELECT entry_id FROM journal_lines GROUP BY entry_id HAVING SUM(debit_cents)<>SUM(credit_cents)')
      .length
  )
    errors.push('Unbalanced journals');
  for (const row of all(
    db,
    `SELECT i.*, COALESCE(SUM(m.quantity),0) ledger_qty, COALESCE(SUM(m.value_delta_cents),0) ledger_value FROM inventory i LEFT JOIN inventory_movements m ON m.product_id=i.product_id AND m.branch_id=i.branch_id AND m.business_id=i.business_id GROUP BY i.product_id,i.branch_id,i.business_id`,
  )) {
    if (row.quantity !== row.ledger_qty || row.value_cents !== row.ledger_value)
      errors.push(`Inventory ledger mismatch: ${row.product_id}`);
  }
  const stockValue = one(db, 'SELECT COALESCE(SUM(value_cents),0) n FROM inventory')!.n;
  const inventoryGl = one(
    db,
    "SELECT COALESCE(SUM(debit_cents-credit_cents),0) n FROM journal_lines WHERE account='Inventory'",
  )!.n;
  if (stockValue !== inventoryGl) errors.push('Inventory does not tie to the general ledger');
  return {
    ok: !errors.length,
    errors,
    auditEvents: one(db, 'SELECT COUNT(*) n FROM audit_logs')!.n,
    checkedAt: now(),
  };
}
