...
export const supplierSchema = z
  .object({
    ...
    account_ref: z.string().max(100).default(''),
...
export function saveSupplier(
  ...
  if (original)
    db.prepare(
      `UPDATE suppliers SET ${Object.keys(row)
        .map((k) => `${k}=?`)
        .join(',')} WHERE id=? AND business_id=?`,
    ).run(...Object.values(row), rid, a.business_id);
  else insert(db, 'suppliers', { id: rid, business_id: a.business_id, ...row, created_at: now() });
  audit(
    db,
    a,
    original ? 'supplier.updated' : 'supplier.created',
    'suppliers',
    rid,
    original,
    row,
...
const purchaseSchema = z
  .object({
    ...
    items: z
      .array(z.object({ product_id: z.string(), quantity: quantityInput, cost: moneyInput }).strict())
      .min(1)
      .max(100),
    reason: reasonInput,
  })
...
export function recordSupplierPayment(
  ...
  const paid =
    one(db, 'SELECT COALESCE(SUM(amount_cents),0) n FROM supplier_payments WHERE purchase_id=?', purchase.id)!
      .n -
    one(
      db,
      'SELECT COALESCE(SUM(amount_cents),0) n FROM supplier_payment_reversals WHERE purchase_id=?',
      purchase.id,
    )!.n;
  const value = cents(input.amount);
  requireThat(
    value > 0 && paid + value <= purchase.total_cents,
    'Payment exceeds the outstanding supplier balance.',
  );
  requireThat(
    one(db, 'SELECT id FROM purchase_receipts WHERE purchase_id=?', purchase.id),
    'Approve receipt of the purchase before paying it.',
  );
  const session = input.method === 'Cash' ? ownSession(db, a) : null;
  if (input.method === 'Cash') {
    const expected = availableCash(db, session!.id);
    requireThat(
      expected >= value,
      'Not enough expected cash in this register for the supplier payment.',
      409,
    );
  }
  const row = {
  ...
  journal(
    ...
    'Supplier payment',
    [
      { account: 'Accounts payable', debit: value },
      { account: paymentAccount(input.method), credit: value },
    ],
    approvalId,
  );
  audit(
...
export function availableCash(db: DB, sessionId: string) {
  ...
  return (
    ...
    one(
      db,
      "SELECT COALESCE(SUM(amount_cents),0) n FROM payments WHERE session_id=? AND method='Cash'",
      sessionId,
    )!.n -
    one(
      db,
      "SELECT COALESCE(SUM(amount_cents),0) n FROM expenses WHERE session_id=? AND method='Cash'",
      sessionId,
    )!.n +
    one(
      db,
      "SELECT COALESCE(SUM(amount_cents),0) n FROM expense_reversals WHERE session_id=? AND method='Cash'",
      sessionId,
    )!.n -
    one(
      db,
      "SELECT COALESCE(SUM(amount_cents),0) n FROM supplier_payments WHERE session_id=? AND method='Cash'",
      sessionId,
    )!.n +
    one(
      db,
      "SELECT COALESCE(SUM(amount_cents),0) n FROM supplier_refunds WHERE session_id=? AND method='Cash'",
      sessionId,
    )!.n +
    one(
      db,
      "SELECT COALESCE(SUM(amount_cents),0) n FROM supplier_payment_reversals WHERE session_id=? AND method='Cash'",
      sessionId,
    )!.n
  );
}
export function postPurchase(db: DB, a: Actor, purchaseId: string, approvalId: string | null = null) {
  demand(a, 'inventory.post');
  const purchase = scoped(db, 'purchases', purchaseId, a);
  requireThat(
    !one(db, 'SELECT id FROM purchase_receipts WHERE purchase_id=?', purchaseId),
  ...
  for (const item of items)
    moveStock(db, a, {
      ...
      value_delta_cents: item.total_cents,
      kind: 'purchase',
      reference: purchase.ref,
      reason: 'Supplier stock received',
      approval_id: approvalId,
    });
  journal(
    db,
...
export function createPurchase(db: DB, a: Actor, input: unknown) {
  ...
  if (b.document_id) {
    ...
    requireThat(doc.user_id === a.id, 'Upload your own supporting document.', 403);
  }
  requireThat(
    new Set(b.items.map((i) => i.product_id)).size === b.items.length,
    'Combine duplicate products in a purchase.',
  );
  const items = b.items.map((i) => {
    const product = scoped(db, 'products', i.product_id, a, false);
    requireThat(product.active, 'Cannot receive an inactive product.');
    const cost = cents(i.cost);
    return {
      id: id('pi_'),
      ...scope(a),
...
export function openingStock(db: DB, a: Actor, input: unknown) {
  demand(a, 'inventory.opening');
  const b = z
    .object({ product_id: z.string(), quantity: quantityInput, reason: reasonInput })
    .strict()
    .parse(input);
  const p = scoped(db, 'products', b.product_id, a, false);
  requireThat(p.cost_cents !== null, 'Enter a buying price before recording opening stock.');
  requireThat(
    !one(
      db,
      'SELECT id FROM inventory_movements WHERE business_id=? AND branch_id=? AND product_id=? LIMIT 1',
      a.business_id,
      a.branch_id,
      p.id,
    ),
    'Opening stock is only allowed before the first stock movement. Use an approved adjustment instead.',
    409,
  );
  const value = total([p.cost_cents * b.quantity]),
    reference = ref('OP');
  const movement = moveStock(db, a, {
    product_id: p.id,
    quantity: b.quantity,
    value_delta_cents: value,
    kind: 'opening',
    reference,
    reason: b.reason,
  });
  journal(db, a, reference, 'Opening inventory', [
    { account: 'Inventory', debit: value },
    { account: 'Opening equity', credit: value },
  ]);
...
export function stockRequest(db: DB, a: Actor, input: unknown) {
  ...
  const b = z
    .object({
      product_id: z.string(),
      kind: z.enum(['stock_count', 'stock_adjustment', 'wastage', 'damaged']),
    ...
    .parse(input);
  const p = scoped(db, 'products', b.product_id, a, false),
    stock = inventoryFor(db, a, p.id);
  if (b.kind === 'stock_count') requireThat(b.quantity >= 0, 'Count must be zero or greater.');
  if (['wastage', 'damaged'].includes(b.kind))
    requireThat(b.quantity > 0, 'Enter the positive quantity lost or damaged.');
  const delta =
    b.kind === 'stock_count'
      ? b.quantity - stock.quantity
      : ['wastage', 'damaged'].includes(b.kind)
        ? -b.quantity
        : b.quantity;
  if (delta > 0)
    requireThat(p.cost_cents !== null || stock.quantity > 0, 'Set a buying price before increasing stock.');
  ...
  const request = newRequest(db, a, {
    ...
    requested_change:
      b.kind === 'stock_count' ? `Counted ${b.quantity} units` : `${delta > 0 ? '+' : ''}${delta} units`,
    original: { product: p.name, stock: stock.quantity, stock_value_cents: stock.value_cents },
    payload: {
      product_id: p.id,
      delta,
      counted: b.kind === 'stock_count' ? b.quantity : null,
...
export function applyStockRequest(db: DB, a: Actor, r: Row) {
  ...
  const p = JSON.parse(r.payload_json),
    stock = inventoryFor(db, a, p.product_id);
  requireThat(
    stock.version === p.expected_inventory_version,
    'Stock changed after this request. Reject it and ask for a fresh count or adjustment.',
    409,
  );
  const delta = p.delta;
  if (delta === 0) {
    audit(db, a, 'inventory.count_confirmed', 'products', p.product_id, stock, stock, r.reason, r.id);
    return;
  }
  let value: number;
  if (r.kind === 'stock_reversal') value = p.value_delta_cents;
    ...
    value =
      stock.quantity > 0
        ? roundRatio(stock.value_cents, delta, stock.quantity)
        : total([p.cost_cents * delta]);
  moveStock(db, a, {
    product_id: p.product_id,
    quantity: delta,
    value_delta_cents: value,
    kind: r.kind === 'stock_reversal' ? 'reversal' : r.kind.replace('stock_', ''),
    reference: r.ref,
    reason: r.reason,
    approval_id: r.id,
  });
  ...
  journal(
    ...
    value >= 0
      ? [
          { account: 'Inventory', debit: value },
          { account: counter, credit: value },
        ]
      : [
          { account: counter, debit: -value },
...
export function installInventory(app: Express, db: DB) {
  app.get('/api/supplier-payments/:id', protect('expenses.create'), (req, res) => {
    const payment = scoped(db, 'supplier_payments', String(req.params.id), req.actor);
    res.json({
      payment,
      purchase: scoped(db, 'purchases', payment.purchase_id, req.actor),
      reversal: one(db, 'SELECT * FROM supplier_payment_reversals WHERE payment_id=?', payment.id),
    });
  });
  app.get('/api/suppliers', protect('suppliers.read'), (req, res) =>
    res.json({
      suppliers: all(
        db,
        'SELECT s.*,(SELECT COUNT(*) FROM products p WHERE p.supplier_id=s.id) product_count FROM suppliers s WHERE s.business_id=? ORDER BY s.name',
        req.actor.business_id,
      ),
    }),
  );
  app.post('/api/suppliers', protect('suppliers.write'), (req, res) =>
    res.status(201).json(mutate(db, req, () => saveSupplier(db, req.actor, req.body))),
  );
  app.patch('/api/suppliers/:id', protect('suppliers.write'), (req, res) =>
    res.json(mutate(db, req, () => saveSupplier(db, req.actor, req.body, String(req.params.id)))),
  );
  app.get('/api/purchases', protect('inventory.receive'), (req, res) =>
    res.json({
      purchases: all(
        db,
        `SELECT p.*,u.name staff_name,CASE WHEN EXISTS(SELECT 1 FROM purchase_reversals WHERE purchase_id=p.id) THEN 'reversed' WHEN pr.id IS NOT NULL THEN 'posted' ELSE COALESCE(ar.status,'pending') END status,
    ...
    WHERE p.business_id=? AND p.branch_id=? ORDER BY p.created_at DESC LIMIT 2000`,
        req.actor.business_id,
        req.actor.branch_id,
      ),
    }),
  );
  app.get('/api/purchases/:id', protect('inventory.receive'), (req, res) => {
    const p = scoped(db, 'purchases', String(req.params.id), req.actor);
    res.json({
      purchase: p,
      items: all(db, 'SELECT * FROM purchase_items WHERE purchase_id=?', p.id),
      payments: all(db, 'SELECT * FROM supplier_payments WHERE purchase_id=?', p.id),
      payment_reversals: all(db, 'SELECT * FROM supplier_payment_reversals WHERE purchase_id=?', p.id),
  ...
  app.post('/api/purchases', protect('inventory.receive'), (req, res) =>
    res.status(201).json(mutate(db, req, () => createPurchase(db, req.actor, req.body))),
  );
  app.post('/api/purchases/:id/payments', protect('expenses.create'), (req, res) => {
    const b = z
      .object({ amount: moneyInput, method: methodInput, reference: z.string().trim().min(3).max(100) })
      .strict()
      .parse(req.body);
    res.status(201).json(
      mutate(db, req, () => ({
        ok: true,
        payment: recordSupplierPayment(
          db,
          req.actor,
          scoped(db, 'purchases', String(req.params.id), req.actor),
          b,
        ),
      })),
    );
  });
  app.get('/api/inventory', protect('inventory.read'), (req, res) =>
    res.json({
      products: productList(db, req.actor),
      movements: all(
        db,
        'SELECT m.*,p.name product_name,p.sku,u.name user_name FROM inventory_movements m JOIN products p ON p.id=m.product_id JOIN users u ON u.id=m.user_id WHERE m.business_id=? AND m.branch_id=? ORDER BY m.created_at DESC LIMIT 2000',
        req.actor.business_id,
        req.actor.branch_id,
      ),
    }),
  );
  app.post('/api/inventory/opening', protect('inventory.opening'), (req, res) =>
    res.status(201).json(mutate(db, req, () => openingStock(db, req.actor, req.body))),
  );
  app.post('/api/inventory/requests', protect('inventory.request'), (req, res) =>
    res.status(201).json(mutate(db, req, () => stockRequest(db, req.actor, req.body))),
  );
}
