...
export const priceSchema = z
  .object({
    ...
    cost: moneyInput.nullable(),
    selling: moneyInput.nullable(),
    wholesale: moneyInput.nullable(),
    promo: moneyInput.nullable(),
    tax_mode: z.enum(['unset', 'none', 'inclusive', 'exclusive']),
    tax_bps: z.number().int().min(0).max(10000),
  })
  .strict();
const priceSnapshot = (p: Row) => ({
  cost_cents: p.cost_cents,
  selling_cents: p.selling_cents,
  wholesale_cents: p.wholesale_cents,
  promo_cents: p.promo_cents,
  tax_mode: p.tax_mode,
  tax_bps: p.tax_bps,
});
export function applyPrices(
  db: DB,
  a: Actor,
  rows: z.infer<typeof priceSchema>[],
  reason: string,
  approvalId: string | null = null,
) {
  demand(a, 'prices.write');
  reasonInput.parse(reason);
  requireThat(
    new Set(rows.map((r) => r.product_id)).size === rows.length,
    'Duplicate product in price batch.',
  );
  for (const input of rows) {
    const row = priceSchema.parse(input),
      original = scoped(db, 'products', row.product_id, a, false);
    requireThat(
      original.version === row.version,
      `${original.name} was updated by another user. Reload the price sheet.`,
      409,
    );
    const next = {
      cost_cents: row.cost === null ? null : cents(row.cost),
      selling_cents: row.selling === null ? null : cents(row.selling),
      wholesale_cents: row.wholesale === null ? null : cents(row.wholesale),
      promo_cents: row.promo === null ? null : cents(row.promo),
      tax_mode: row.tax_mode,
      tax_bps: row.tax_bps,
    };
    for (const field of ['selling_cents', 'wholesale_cents', 'promo_cents'] as const)
      requireThat(
        next[field] === null || next[field]! > 0,
        'Selling, promotional and wholesale prices must be greater than zero.',
      );
    requireThat(
      !['none', 'unset'].includes(next.tax_mode) || next.tax_bps === 0,
      'A zero tax rate is required for no-tax/unconfigured products.',
    );
    ...
    insert(db, 'price_history', {
      ...
      user_id: a.id,
      previous_json: JSON.stringify(previous),
    ...
    db.prepare(
      'UPDATE products SET cost_cents=?,selling_cents=?,wholesale_cents=?,promo_cents=?,tax_mode=?,tax_bps=?,version=version+1,updated_at=? WHERE id=? AND business_id=?',
    ).run(
      next.cost_cents,
      next.selling_cents,
      next.wholesale_cents,
      next.promo_cents,
      next.tax_mode,
      next.tax_bps,
      now(),
...
const productSchema = z
  .object({
    ...
    min_stock: z.number().int().min(0).max(100000).default(0),
    reorder_level: z.number().int().min(0).max(100000).default(0),
    active: z.boolean().default(true),
    image: z
      .string()
      .max(150)
      .regex(/^(|\/api\/documents\/[a-zA-Z0-9_-]+)$/)
      .default(''),
...
export function productList(db: DB, a: Actor) {
  const rows = all(
    db,
    `${productSelect} WHERE p.business_id=? ORDER BY p.name,p.size`,
    a.branch_id,
    a.business_id,
  );
  ...
  if (a.role_id === 'cashier')
    for (const row of rows) {
      ...
      delete row.stock_value_cents;
    }
  return rows;
}
export function installProducts(app: Express, db: DB) {
  app.get('/api/catalogue/meta', protect('products.read'), (req, res) =>
    res.json({
      categories: all(
        db,
      ...
      brands: all(db, 'SELECT * FROM brands WHERE business_id=? ORDER BY name', req.actor.business_id),
      subcategories: CATEGORY_CONFIG,
    }),
  );
  app.get('/api/products', protect('products.read'), (req, res) =>
    res.json({ products: productList(db, req.actor) }),
  );
  app.get('/api/products/:id/history', protect('prices.write'), (req, res) => {
    scoped(db, 'products', String(req.params.id), req.actor, false);
    res.json({
      history: all(
        db,
        'SELECT h.*,u.name user_name FROM price_history h JOIN users u ON u.id=h.user_id WHERE h.product_id=? AND h.business_id=? ORDER BY h.created_at DESC',
        req.params.id,
        req.actor.business_id,
      ),
    });
  ...
  app.post('/api/products/prices', protect('prices.write'), (req, res) => {
    const body = z
      .object({ rows: z.array(priceSchema).min(1).max(500), reason: reasonInput })
      .strict()
      .parse(req.body);
  ...
  function saveProduct(a: Actor, input: unknown, productId?: string) {
    ...
    const lookup = (table: string, name: string) => {
      let row = one(db, `SELECT id FROM ${table} WHERE business_id=? AND name=?`, a.business_id, name);
      if (!row) {
        const rid = id();
        insert(db, table, { id: rid, business_id: a.business_id, name });
        row = { id: rid };
      }
      return row.id;
    };
    const row = {
      name: b.name,
      brand_id: lookup('brands', b.brand),
      category_id: lookup('categories', b.category),
      subcategory: b.subcategory,
      sku: b.sku || original?.sku || `KIL-${id().slice(0, 8).toUpperCase()}`,
      size: b.size,
      unit: b.unit,
    ...
    if (original)
      db.prepare(
        `UPDATE products SET ${Object.keys(row)
          .map((k) => `${k}=?`)
          .join(',')},version=version+1 WHERE id=? AND business_id=?`,
    ...
    audit(
      db,
      a,
      original ? 'product.updated' : 'product.created',
      'products',
      rid,
  ...
  app.post('/api/products', protect('products.write'), (req, res) =>
    res.status(201).json(mutate(db, req, () => saveProduct(req.actor, req.body))),
  );
  app.patch('/api/products/:id', protect('products.write'), (req, res) =>
    res.json(mutate(db, req, () => saveProduct(req.actor, req.body, String(req.params.id)))),
  );
  app.get('/api/workspace/summary', protect(), (req, res) => {
    const a = req.actor;
    res.json({
      products: one(db, 'SELECT COUNT(*) n FROM products WHERE business_id=?', a.business_id)!.n,
      pending: can(a, 'approvals.read')
        ? one(
            db,
