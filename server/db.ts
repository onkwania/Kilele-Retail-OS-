...
export function createDb(path = ':memory:') {
  ...
  if (path !== ':memory:') {
    ...
    // Do not expose financial data, password hashes or session metadata to other host users.
    for (const name of [file, file + '-wal', file + '-shm']) if (existsSync(name)) chmodSync(name, 0o600);
  }
...
export function actorFor(db: DB, userId: string): Actor | null {
  ...
  if (!user) return null;
  const perms = all(db, 'SELECT permission_id FROM role_permissions WHERE role_id=?', user.role_id).map(
...
export function bootstrap(
  db: DB,
  details: { name: string; email: string; password: string; business?: string },
) {
  ...
  return db
    .transaction(() => {
      const businessId = id('biz_'),
        branchId = id('br_'),
        userId = id('usr_');
      insert(db, 'businesses', {
        id: businessId,
        name: details.business ?? 'My beverage business',
        created_at: now(),
      });
      insert(db, 'branches', {
        id: branchId,
