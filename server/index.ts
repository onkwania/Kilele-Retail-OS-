...
import { engine } from './postgres/db.js';
...
/** The SQLite ledger: unchanged behaviour for the demo and every deployment running today. */
function startSqliteServer() {
...
// The storage engine is decided once, here. In postgres mode the SQLite path is never entered, so
// no SQLite file is opened or created; in sqlite mode the PostgreSQL pool is never constructed.
if (engine() === 'postgres') {
  startPostgresServer({
    production,
    preview,
